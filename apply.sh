...
original_author="vladiulianbogdan"
original_name="my_flask_test"
original_urlname="my-flask-test"
original_description="Awesome my_flask_test created by vladiulianbogdan" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/my_flask_test "${name}"
...
then
    ...
    echo "then run 'my_flask_test' or 'python -m my_flask_test'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'my_flask_test' or 'python -m my_flask_test' or 'poetry run my_flask_test'"
fi
