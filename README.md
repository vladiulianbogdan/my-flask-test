...
- 🎯 Entry points to execute your program using `python -m <my_flask_test>` or `$ my_flask_test` with basic CLI argument parsing.
...
---
# my_flask_test Flask Application

Awesome my_flask_test created by vladiulianbogdan
...
```bash
git clone https://github.com/vladiulianbogdan/my-flask-test my_flask_test
cd my_flask_test
make install
```
...
```bash
my_flask_test create-db   # run once
my_flask_test populate-db  # run once (optional)
my_flask_test add-user -u admin -p 1234  # ads a user
my_flask_test run
```
